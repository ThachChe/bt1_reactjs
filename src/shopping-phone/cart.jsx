...
export default function Cart(props) {
  ...
  const { listCart, onDeleteToCart, handleUpdateQty } = props;
      ...
      return (
        <tr key={phone.maSP} className="border-b">
          ...
          <td className="px-4 py-3">
            <button onClick={() => handleUpdateQty(phone.maSP, false)}>
              -
            </button>
            {phone.soLuong}
            <button onClick={() => handleUpdateQty(phone.maSP, true)}>+</button>
          </td>
          <td className="px-4 py-3">{phone.giaBan * phone.soLuong}</td>
          <td className="px-4 py-3">
            <button
              onClick={() => onDeleteToCart(phone.maSP)}
              type="button"
